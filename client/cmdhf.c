...
uint8_t iclass_CRC_check(bool isResponse, uint8_t *data, uint8_t len) {
    if (len < 4)
        return 2; // CRC commands (and responses) are all at least 4 bytes
...
#define MAX_TOPAZ_READER_CMD_LEN 16
    ...
    if ((*data_len != 1) || (frame[0] == TOPAZ_WUPA) || (frame[0] == TOPAZ_REQA))
        return false;
...
uint16_t printTraceLine(uint16_t tracepos, uint16_t traceLen, uint8_t *trace, uint8_t protocol, bool showWaitCycles, bool markCRCBytes) {
    ...
    if (tracepos + sizeof(uint32_t) + sizeof(uint16_t) + sizeof(uint16_t) > traceLen)
        return traceLen;
    ...
    for (int j = 0; j < num_lines; j++) {
        if (j == 0) {
            PrintAndLog(" %10d | %10d | %s |%-64s | %s| %s", (timestamp - first_timestamp), (EndOfTransmissionTimestamp - first_timestamp),
                        (isResponse ? "Tag" : "Rdr"), line[j], (j == num_lines - 1) ? crc : "    ", (j == num_lines - 1) ? explanation : "");
        } else {
            PrintAndLog("            |            |     |%-64s | %s| %s", line[j], (j == num_lines - 1) ? crc : "    ",
                        (j == num_lines - 1) ? explanation : "");
        }
    }

    if (is_last_record(tracepos, trace, traceLen))
        return traceLen;

    if (showWaitCycles && !isResponse && next_record_is_response(tracepos, trace)) {
        uint32_t next_timestamp = *((uint32_t *)(trace + tracepos));
        PrintAndLog(" %10d | %10d | %s | fdt (Frame Delay Time): %d", (EndOfTransmissionTimestamp - first_timestamp), (next_timestamp - first_timestamp), "   ",
...
int CmdHFStandalone(const char *Cmd) {
    PrintAndLog("\n Launching HF Standalone mode...\n");
    UsbCommand c = {CMD_HF_STANDALONE, {0, 0, 0}};
    SendCommand(&c);
    return 0;
}

static command_t CommandTable[] = {{"help", CmdHelp, 1, "This help"},
                                   ...
                                   {"standalone", CmdHFStandalone, 0, "Start HF standalone mode without button helding"},
                                   {NULL, NULL, 0, NULL}};

...
void SetSerialPort(serial_port *new_port) { port = new_port; }

serial_port *GetSerialPort() { return port; }

void SetOffline(bool new_offline) { offline = new_offline; }
...
bool WaitForResponseTimeout(uint64_t cmd, UsbCommand *response, size_t ms_timeout) { return WaitForResponseTimeoutW(cmd, response, ms_timeout, true); }

bool WaitForResponse(uint64_t cmd, UsbCommand *response) { return WaitForResponseTimeout(cmd, response, -1); }
